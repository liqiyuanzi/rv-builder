...
const getVueStylePath = (filePath, ext, index) => {
    return common_1.replaceExt(filePath, `${index || ''}.${ext}`);
};
...
async function default_1(filepath) {
    ...
    tasks.push(new Promise((resolve) => {
        ...
        js_1.default(jsPath).then(resolve); //eslint-disable-line
    }));
    styles.forEach((style, index) => {
        tasks.push(new Promise((resolve) => {
            ...
            style_1.default(cssPath).then(resolve); //eslint-disable-line
        }));
    });
